...
class _DataclassParams:
    ...
    def __init__(
        self, init, repr, eq, order, unsafe_hash, frozen, match_args, kw_only, slots, weakref_slot
    ):
        self.init = init
        self.repr = repr
        self.eq = eq
...
def _field_init(f, frozen, globals, self_name, slots):
    ...
    if f.default_factory is not MISSING:
        if f.init:
            ...
            value = f"{default_name}() " f"if {f.name} is __dataclass_HAS_DEFAULT_FACTORY__ " f"else {f.name}"
        else:
...
def _process_class(
    cls, init, repr, eq, order, unsafe_hash, frozen, match_args, kw_only, slots, weakref_slot
):
